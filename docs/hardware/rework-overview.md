...
This section documents the CW paddle rework for use with the CW firmware, and the custom cables which may be used instead of a rework.
...
=== "UV-K5 v1 original"
    This page covers the original UV-K5 hardware rework.
...
| --- | --- | --- |
| Avoid opening the radio | Cable route ([CEC](cec-cable.md) for v1, USB-C TRS for v3/K1) | Works with a purpose-built cable only |
| Use any off-the-shelf paddle cable | Perform the rework | Requires opening the case and modifying resistors |

## Shared notes for all models

!!! danger "Important limitation"
    When CW mode is used with port-based input modes or the CEC cable mode, the serial port will not behave correctly while the main firmware is running. That affects CHIRP programming. Switch out of CW or out of the port input mode before using a memory programmer.
